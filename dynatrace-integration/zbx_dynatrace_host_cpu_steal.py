...
try:
    metric_selector = f"builtin:host.cpu.steal:filter(eq(dt.entity.host,{HOST_ID})):avg"
    ...
    if data and data[0].get('data'):
        ...
        steal_time = round(values[-1], 2) if values else 0
        
    print(json.dumps({"cpu_steal_time_percent": steal_time}))
